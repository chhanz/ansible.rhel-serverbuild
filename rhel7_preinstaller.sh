...
# Set Hostname
echo -e "${RED}SET HOSTNAME${NC}" |tee -a $LOGFILE
echo -n "Input Hostname (if set later input 1) : "
read hname
if [ "a$hname" == "a1" ]; then
 echo "set hostname later" |tee -a $LOGFILE
else
  hostnamectl set-hostname $hname --static
fi

# Backup ifcfg files
echo -e "${RED}BACKUP IFCFG FILES${NC}" |tee -a $LOGFILE
mkdir /etc/sysconfig/network-scripts/bak
cp -pH /etc/sysconfig/network-scripts/ifcfg-* /etc/sysconfig/network-scripts/bak/
ls -l /etc/sysconfig/network-scripts/bak |tee -a $LOGFILE
